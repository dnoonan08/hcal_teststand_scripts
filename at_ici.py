		...
		for phase in range(16):
			...
			if phase_result:
				...
				for link in links:
					...
					if v: print "==============="
					uhtr_out = uhtr.get_triggered_data(ts=ts, crate=at.be_crate, slot=at.be_slot, i_link=link.n, n_events=n_events, f="at_ici", script=True)
					if uhtr_out:
#						for ch in range(4):
...
#	th1 = make_histograms(data)
	...
	# Write output:
	at.canvas.SetCanvasSize(750, 500)
#	at.canvas.Divide(6)
	at.canvas.Divide(6, 4)
...
def create_plots():
	...
	for i_qie in range(1, 25):
		...
		th2[i_qie] = TH2F("h{0}".format(i_qie), "", 15, 0, 15, 64, 0, 64)
		for phase in range(16):
