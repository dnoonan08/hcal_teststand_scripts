...
def read_links(ip): # Looks at the spy dumps of all of the active channels and parses out the QIE card's unique ID and fiber information corresponding to the target qie channels
	active_links = uhtr.find_links(ip)
...
def read_links2(ip):		# Should replace the above function with this one.
	active_links = uhtr.find_links(ip)
	uhtr_map = uHTRlinkMap()
	...
	for link_ in uhtr_map.links:
		if uhtr_map.links.index(link_) in active_links:
			#print "link",uhtr_map.links.index(link_),"on..."
			link_.on = True
			data = uhtr.get_data_parsed(ip, 3, uhtr_map.links.index(link_))		# grab spy dump and parse information about this link
			if len(set(data["fiber"])) == 1:
				link_.qie_fiber = data["fiber"][0]
			if len(set(data["half"])) == 1:
				link_.qie_fiber = data["half"][0]
#			for line in uhtr_read["output"].split("\n"):
#				#print line
#				if line.find("TOP fiber") != -1 : 
#					#print line.split(" ")[5]
	...
	for ip in ts.uhtr_ips:
		...
		for link in active_links:
			if link.on:
				...
				link.Print()
#				uhtr_map2.links[uhtr_map.links.index(link)].Print()		# Should be the same as above.
				print ""
